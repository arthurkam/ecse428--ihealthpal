@extends('layouts.main')
@section('content')

<h2>Meal Plan Suggestions</h2>

<!-- Nav tabs -->
<ul class="nav nav-tabs">
  <li class="active"><a href="#massGain1" data-toggle="tab">Mass Gain Meal Plan 1</a></li>
  <li><a href="#massGain2" data-toggle="tab">Mass Gain Meal Plan 2</a></li>
  <li><a href="#weightLoss1" data-toggle="tab">Weight Loss Meal Plan 1</a></li>
  <li><a href="#weightLoss2" data-toggle="tab">Weight Loss Meal Plan 2</a></li>
</ul>

<!-- Tab panes -->
<div class="tab-content">

<! http://www.menshealth.co.uk/food-nutrition/muscle-foods/eat-all-week-to-build-muscle-332752 >
  <div class="tab-pane active" id="massGain1">
    <div class="row">
    
    <div class="panel panel-default">
	  <!-- Default panel contents -->
	  <div class="panel-heading"><font size="5"><b>Mass Gain Meal Plan 1</b></font></div>
	  <div class="panel-body">
	    <p><font size="3">After a workout your body cries out for additional sustenance to repair the damage you’ve done to your muscles but trying to finance this with the standard three meals a day won’t yield any results. Here’s a complete muscle-building eating plan from sports nutritionist.</font></p>
	  </div>
	
	  <!-- Table -->
	  <table class="table">
	    <tr>
	      <td width="50">     </td>
		  <td width="100"><b>Day 1</b></td>
		  <td width="100"><b>Day 2</b></td> 
		  <td width="100"><b>Day 3</b></td>
		  <td width="100"><b>Day 4</b></td>
		  <td width="100"><b>Day 5</b></td>
		  <td width="100"><b>Day 6</b></td>
		  <td width="100"><b>Day 7</b></td>
		</tr>
		<tr>
		  <td><b>Breakfast</b></td>
		  <td>
		  <li>3-egg omelette with:<br>Sliced Mushrooms,<br>Olive Oil,<br>Diced Onions,<br>Diced Tomatoes</li>
		  </td>
		  <td>
		  <li>Handful of Oats</li>
		  <li>250ml Skimmed Milk</li>
		  <li>10ml Honey</li>
		  <li>Whey Protein</li>
		  <li>Cook and mix together</li></td> 
		  <td>
		  <li>2 Scrambled Eggs</li>
		  <li>1 Can of Baked Beans</li>
		  <li>2 Slices of Toast (Rye Bread)</li>
		  <li>200ml Orange Juice</li>
		  </td>
		  <td>
		  <li>Smoothies made from:<br>Whey concentrate powder<br>Handful of Blueberries<br>Handful of Raspberries<br>40ml Plain Yogurt<br>1tbsp Rolled Oats<br>1 Banana</li>
		  </td>
		  <td>
		  <li>4 Weetabix with:<br>250ml Skimmed Milk<br>Fruit Yogurt<br>Handful of Blackberries</li>
		  </td>
		  <td>
		  <li>Smoothies made from:<br>Whey Protein<br>1 Ripe Banana<br>300ml Water<br>Pinch of Ground Cinnamon<br>20g Ground Almonds</li>
		  </td>
		  <td>
		  <li>3 Poached Eggs</li>
		  <li>Low-fat Sausages</li>
		  <li>2 Slices of Toast</li>
		  <li>1 Tomato</li>
		  </td>
		</tr>
		<tr>
		  <td><b>Midmorning Snack</b></td>
		  <td>
		  <li>150ml Plain Yogurt with:<br>Blueberries<br>Drizzle with Manuka honey<br>Ground mixed seeds and nuts</li>
		  </td>
		  <td>
		  <li>2 Boiled Eggs</li>
		  <li>1 Banana</li>
		  </td>
		  <td>
		  <li>1 Tine of Sardines on Oat Crackers</li>
		  <li>1 Pear</li>
		  </td>
		  <td>
		  <li>2 Boiled Eggs</li>
		  <li>1 Apple</li>
		  </td>
		  <td>
		  <li>1 Avocado Topped with:<br>Handful of King Prawns<br>Sliced Mango<br>Drizzle with Lemon Juice</li>
		  </td>
		  <td>
		  <li>1 Toasted Bagel with:<br>1 Mashed Banana<br>Peanut Butter<br>Manuka Honey</li>
		  </td>
		  <td>
		  <li>Toasted Cheese and Tomato on<br>2 Slices of Rye Bread</li>
		  <li>1 Apple</li>
		  </td>
		</tr>
		<tr>
		  <td><b>Lunch</b></td>
		  <td>
		  <li>Chicken Breast</li>
		  <li>Avocado</li>
		  <li>Tomato</li>
		  <li>Cheese</li>
		  <li>Baguette</li>
		  </td>
		  <td>
		  <li>300g Fillet Steak</li>
		  <li>Mashed Potato</li>
		  <li>Spinach</li>
		  </td>
		  <td>
		  <li>Small Jacket Potato with:<br>Baked Beans<br>Cheese<br>Sliced Pork Sausages<br>Chillies</li>
		  </td>
		  <td>
		  <li>2 Turkey Breast Fillets</li>
		  <li>Cranberry</li>
		  <li>Cheese</li>
		  <li>Rocket Wraps</li>
		  </td>
		  <td>
		  <li>Small Jacket Potato with:<br>Tuna<br>Sweetcorn<br>Low-fat Mayo<br>Chillies<br>Cheese</li>
		  <li>1 Apple</li>
		  </td>
		  <td>
		  <li>Peppered Mackerel</li>
		  <li>Avocado</li>
		  <li>Mixed Salad Baguette</li>
		  </td>
		  <td>
		  <li>Mixed Sushi Box</li>
		  </td>
		</tr>
		<tr>
		  <td><b>Afternoon Snack</b></td>
		  <td>
		  <li>Protein bar</li>
		  <li>1 Banana</li>
		  </td>
		  <td>
		  <li>Mixed Seeds, Dried Fruits and Nuts</li>
		  <li>200ml of cottage cheese</li>
		  </td>
		  <td>
		  <li>Plain Sugar-Free Yogurt with:<br>Sliced Banana and Strawberries<br>Ground Mixed Nuts and Seeds</li>
		  </td>
		  <td>
		  <li>150g of cottage cheese</li>
		  <li>Diced Pineapple on Rice or Oat Cakes</li>
		  </td>
		  <td>
		  <li>1 Grilled Seasoned Chicken Breast with 2 small Beetroots</li>
		  </td>
		  <td>
		  <li>2 Boiled Eggs</li>
		  <li>2 Slices of Rye Bread</li>
		  <li>1 Apple</li>
		  </td>
		  <td>
		  <li>Oatcakes with:<br>1 Avocado<br>Sliced Proscuitto Ham<br>Salsa</li>
		  </td>
		</tr>
		<tr>
		  <td><b>Dinner</b></td>
		  <td>
		  <li>Tuna Steak with:<br>Stir-fried mixed veggies in olive oil<br>Soba noodles</li>
		  </td>
		  <td>
		  <li>Chicken and Prawn Curry with Soba Noodles</li>
		  </td>
		  <td>
		  <li>Lean Minced Beef Chilli with Mixed Veggies, Beans and Basmati Rice</li>
		  </td>
		  <td>
		  <li>Lemon Sole Fillet</li>
		  <li>2 Handfuls of Baby Potatoes</li>
		  <li>Mixed Vegetables</li>
		  </td>
		  <td>
		  <li>4 Lamb Chops</li>
		  <li>100g Boiled Potatoes</li>
		  <li>Mixed Vegetables</li>
		  </td>
		  <td>
		  <li>2 Grilled Chicken Breasts Seasoned with Cajun Pepper</li>
		  <li>1 Cup of Couscous</li>
		  <li>Mixed Roasted Vegetables</li>
		  </td>
		  <td>
		  <li>2 Grilled Chicken Breasts with:<br>Diced Radishes<br>Green Salad<br>Couscous<br>Dress with Olive Oil and Balsamic Vinegar</li>
		  </td>
		</tr>
	  </table>
	</div>

	</div>
  </div>

<! http://www.bodybuilding.com/fun/mohr60.htm#1 >  
  <div class="tab-pane" id="massGain2">
    <div class="row">
    
    <div class="panel panel-default">
	  <!-- Default panel contents -->
	  <div class="panel-heading"><font size="5"><b>Mass Gain Meal Plan 2</b></font></div>
	  <div class="panel-body">
	    <p><font size="3">Eating 4000 calories each day may make you feel like a bear that is getting ready to hibernate during the winter, but when you build serious muscle than when the small amount of fat storage that comes along with gaining extreme muscle can be hidden underneath your winter clothes. Keep your eye on that mirror; there's no better way to monitor your gains.</font></p>
	  </div>
	
	  <!-- Table -->
	  <table class="table">
	    <tr>
	      <td width="50">     </td>
		  <td width="100"><b>Day 1</b></td>
		  <td width="100"><b>Day 2</b></td> 
		  <td width="100"><b>Day 3</b></td>
		  <td width="100"><b>Day 4</b></td>
		  <td width="100"><b>Day 5</b></td>
		  <td width="100"><b>Day 6</b></td>
		  <td width="100"><b>Day 7</b></td>
		</tr>
		<tr>
		  <td><b>Breakfast</b></td>
		  <td>
		  <li>1.5 Cups Raw Oatmeal</li>
		  <li>1 Cup Skimmed Milk</li>
		  <li>1/2 Cup Dried Cranberries or Raisins</li>
		  <li>1TBS Flax Oil</li>
		  </td>
		  <td>
		  <li>4 Whole Grain Frozen Waffles</li>
		  <li>2 TBS Pure Maple Syrup</li>
		  <li>1 Cup Low-fat Cottage Cheese</li>
		  <li>1 Cup Fresh Strawberries</li>
		  </td>
		  <td>
		  <li>Vegetable Omelet</li>
		  <li>2 Slices 12-Grain Bread</li>
		  <li>1 Cup Non-fat Milk</li>
		  <li>1 Fresh Orange</li>
		  </td>
		  <td>
		  <li>Oat Bran Waffles</li>
		  <li>1 Cup Skimmed Milk</li>
		  </td>
		  <td>
		  <li>Buckwheat Pancake</li>
		  <li>2 Cups of Skimmed Milk</li>
		  <li>1 Cup Fresh Blueberries</li>
		  </td>
		  <td>
		  <li>Breakfast Burritos</li>
		  <li>1 Medium Grapefruit</li>
		  </td>
		  <td>
		  <li>1 Multi-Grain Bagel, Topped with 2 TBS Reduced Cream Cheese and 4oz Smoked Salmon</li>
		  <li>1 Pear</li>
		  </td>
		</tr>
		<tr>
		  <td><b>Midmorning Snack</b></td>
		  <td>
		  <li>1 Cup Skimmed Milk</li>
		  <li>1 Large Piece of Fruit with 1 TBS natural peanut Butter</li>
		  <li>1 Low-fat Mozzarella Stick</li>
		  </td>
		  <td>
		  <li>Peanut Butter, Banana, and Honey Sandwich</li>
		  <li>2 Pieces Whole Grain Bread</li>
		  <li>2 TBS Banana</li>
		  <li>1 Whole Banana</li>
		  <li>2 TBS Honey</li>
		  <li>2 Cups Non-fat Milk</li>
		  </td>
		  <td>
		  <li>1 Can Tuna fish</li>
		  <li>2 Cups cooked Brown Rice</li>
		  <li>Salsa to taste</li>
		  <li>1 Medium Plum</li>
		  </td>
		  <td>
		  <li>Protein Shake</li>
		  <li>1 Cup Skimmed Milk</li>
		  <li>2 Large Pieces of Fruit</li>
		  </td>
		  <td>
		  <li>Protein Shake</li>
		  <li>1 Cup Skimmed Milk</li>
		  <li>2 Large Pieces of Fruit</li>
		  </td>
		  <td>
		  <li>2 Cups Oat Bran</li>
		  </td>
		  <td>
		  <li>Whole Grain, hydrogenated Oil Free Crackers</li>
		  <li>1 Cup Low-fat Cottage Cheese</li>
		  <li>1 Orange</li>
		  </td>
		</tr>
		<tr>
		  <td><b>Lunch</b></td>
		  <td>
		  <li>2 Cups Egg Salad on 2 Whole Wheat Pitas</li>
		  <li>1 Banana</li>
		  </td>
		  <td>
		  <li>Pasta Vegetable Medley</li>
		  <li>1 Cup Non-fat Milk</li>
		  <li>Spinach</li>
		  </td>
		  <td>
		  <li>Turkey and Cheese Sandwich</li>
		  <li>1 Small Bunch Red Grapes</li>
		  </td>
		  <td>
		  <li>Tuna and Cheese Sandwich</li>
		  <li>1 Large Pear</li>
		  </td>
		  <td>
		  <li>Ham and Cheese Sandwich</li>
		  <li>1 Large Apple</li>
		  </td>
		  <td>
		  <li>2 Frozen Organic Burrito</li>
		  <li>Carrot Sticks</li>
		  </td>
		  <td>
		  <li>2 Slices whole Grain Bread with 6oz Ham, 2oz Reduced Fat Cheese, sliced Tomato and Lettuce, and Honey Mustards</li>
		  <li>Veggie Sticks</li>
		  <li>1 Cup Low-fat Milk</li>
		  </td>
		</tr>
		<tr>
		  <td><b>Afternoon Snack</b></td>
		  <td>
		  <li>1 Cup Low-fat Vanilla Yogurt</li>
		  <li>1 Cup Fat-free Cottage Cheese</li>
		  <li>1 Cup Blueberries</li>
		  <li>2 TBS Wheat Germ Oil</li>
		  <li>1 TBS Honey</li>
		  </td>
		  <td>
		  <li>Protein Shake with 1 cup Non-fat Milk, 1 Cup Frozen Fruits, and 2 TBS Flax Oil (Add water to desired consistency)</li>
		  </td>
		  <td>
		  <li>Favorite Meal Replacement Powder</li>
		  <li>1 Cup Skimmed Milk</li>
		  <li>2 Cups Frozen Fruit</li>
		  <li>(Add water to desired consistency)</li>
		  </td>
		  <td>
		  <li>Protein Shake</li>
		  <li>1 Cup Skimmed Milk</li>
		  <li>2 Large Pieces of Fruit</li>
		  </td>
		  <td>
		  <li>Protein Shake</li>
		  <li>1 Cup Skimmed Milk</li>
		  <li>2 Large Pieces of Fruit</li>
		  </td>
		  <td>
		  <li>Triple Decker Peanut Butter and Banana</li>
		  <li>1 Cup Organic Black Bean Soup</li>
		  </td>
		  <td>
		  <li>Favorite Meal Replacement Powder with 2 Cups Low-fat Milk, 2 Cups Frozen Fruit, and 1 TBS Flax Oil (Add water to desire thickness)</li>
		  </td>
		</tr>
		<tr>
		  <td><b>Dinner</b></td>
		  <td>
		  <li>6oz Grilled Salmon</li>
		  <li>1 Large Sweet Potato</li>
		  <li>1 Cup Cut Green Beans</li>
		  <li>1 Cup Skimmed Milk</li>
		  </td>
		  <td>
		  <li>8oz Turkey Breast</li>
		  <li>2 Large Sweet Potatoes</li>
		  <li>1 Cup Collard Greens or Swiss Chard</li>
		  </td>
		  <td>
		  <li>16oz Grilled Pork Chop</li>
		  <li>1 Cup Broccoli</li>
		  <li>2 Cups Whole Grain, Brown Rice</li>
		  </td>
		  <td>
		  <li>1 Bowl Mexican Chicken Chill</li>
		  </td>
		  <td>
		  <li>1 Homemade Burger on Whole Wheat Bun</li>
		  <li>1 Cup Skimmed Milk</li>
		  </td>
		  <td>
		  <li>6oz Tuna Steak</li>
		  <li>1 Cup Whole Wheat Pastas</li>
		  <li>1 Cup Low-fat Milk</li>
		  </td>
		  <td>
		  <li>1 Homemade 6oz Sirloin Burger on Whole Wheat Bun</li>
		  <li>2 Cups Steamed Mixed Vegetables</li>
		  <li>1 Large Sweet Potato</li>
		  <li>1 Cup Skimmed Milk</li>
		  </td>
		</tr>
		<tr>
		  <td><b>After Dinner Snack</b></td>
		  <td>
		  <li>Peanut Butter Smoothie</li>
		  </td>
		  <td>
		  <li>High Protein Pudding</li>
		  </td>
		  <td>
		  <li>Protein Shake</li>
		  <li>1 Large Piece of Fruit</li>
		  </td>
		  <td>
		  <li>Protein Shake</li>
		  <li>1 Large Piece of Fruit</li>
		  </td>
		  <td>
		  <li>Protein Shake</li>
		  <li>1 Large Piece of Fruit</li>
		  </td>
		  <td>
		  <li>Favorite Meal Replacement Powder with 2 Cups Low-fat Milk, 2 Cups Frozen Fruit, and 1 TBS Flax Oil (Add water to desire thickness)</li>
		  </td>
		  <td>
		  <li>1 Cup Low-fat Cottage Cheese with 1 Cup Mixed Fruit Canned in own Juice</li>
		  </td>
		</tr>
	  </table>
	</div>

    
	</div>
  </div>
  
  <! http://www.doctoroz.com/videos/7-day-dash-diet-meal-plan?page=2 >
  <div class="tab-pane" id="weightLoss1">
  <div class="row">
	  <div class="panel panel-default">
	  <!-- Default panel contents -->
	  <div class="panel-heading"><font size="5"><b>Weight Loss Meal Plan 1</b></font></div>
	  <div class="panel-body">
	    <p><font size="3">The overall goal of this meal plan is to lower your consumption of sodium, which aids in lowering your blood pressure. Since the diet focuses on eating the right foods with the right portions, it's also effective for short- and long-term weight loss</font></p>
	  </div>
	
	  <!-- Table -->
	  <table class="table">
	    <tr>
	      <td width="50">     </td>
		  <td width="100"><b>Day 1</b></td>
		  <td width="100"><b>Day 2</b></td> 
		  <td width="100"><b>Day 3</b></td>
		  <td width="100"><b>Day 4</b></td>
		  <td width="100"><b>Day 5</b></td>
		  <td width="100"><b>Day 6</b></td>
		  <td width="100"><b>Day 7</b></td>
		</tr>
		<tr>
		  <td><b>Breakfast</b></td>
		  <td>
		  <li>Hard-boiled Egg</li>
		  <li>1-2 slices Canadian bacon</li>
		  <li>6 ounces tomato juice, low-sodium</li>
		  </td>
		  <td>
		  <li>Mini-Egg Beaters Southwestern Style omelet</li>
		  <li>4-6 ounces tomato juice, low-sodium</li></td> 
		  <td>
		  <li>Scrambled eggs</li>
		  <li>1-2 slices Canadian bacon</li>
		  <li>4-6 ounces diet cranberry juice</li></td>
		  <td>
		  <li>3/4 cup Wheaties</li>
		  <li>8 ounces skim milk</li>
		  <li>4-6 ounces strawberries or raspberries</li></td>
		  <td>
		  <li>Hot chocolate (8 ounces skimmed milk + 1 heaping teaspoon unsweetened cocoa + 2 packets Splenda)</li>
		  <li>1-2 hard-boiled eggs</li>
		  <li>6-8 ounces light cranberry juice</li>
		  <li>4-6 ounces strawberries</li></td>
		  <td>
		  <li>1/2 cup oatmeal with toppings you like</li>
		  <li>1/2 banana</li>
		  <li>4-6 ounces tomato juice (low sodium)</li>
		  <li>Latte (8 ounces skim milk, 2 ounces espresso)</li></td>
		  <td>
		  <li>1-3 scrambled eggs</li>
		  <li>1 slice whole-wheat toast</li>
		  <li>1 tablespoon jelly or jam</li>
		  <li>4-6 ounces orange juice</li>
		  <li>Latte or 8 ounces skim milk</li></td>
		</tr>
		<tr>
		  <td><b>Midmorning Snack</b></td>
		  <td><li>1 stick light cheese</li></td>
		  <td>
		  <li>1 light cheese wedge</li>
		  <li>6 grape tomatoes</li></td> 
		  <td>
		  <li>4 ounces raspberry light yogurt (nonfat)</li>
		  <li>23 almonds (1/4 cup)</li></td>
		  <td>
		  <li>1-2 light cheese wedges</li>
		  <li>Grape tomatoes</li></td>
		  <td>
		  <li>6 ounces key lime light yogurt (nonfat)</li>
		  <li>10 ounces almonds</li></td>
		  <td>
		  <li>1 stick light cheese</li>
		  <li>Baby carrots</li></td>
		  <td>
		  <li>4-6 ounces blueberries</li>
		  <li>10 almonds</li></td>
		</tr>
		<tr>
		  <td><b>Lunch</b></td>
		  <td><li>Tuna Salad</li>
		  <li>Cherry tomatoes</li>
		  <li>Small side salad (Italian Dressing/Vinegar Dressing)</li>
		  <li>Strawberry Jell-O cup, sugar free</li></td>
		  <td>
		  <li>2-3 Turkey-Swiss-roll-ups</li>
		  <li>1/2-1 cup coleslaw</li>
		  <li>Raw snow peas OR sugar snap pea pods</li>
		  <li>Orange Jell-O cup, sugar free</li></td> 
		  <td>
		  <li>Cold fried chicken breast (No skin or coating)</li>
		  <li>Coleslaw</li>
		  <li>Baby carrots</li>
		  <li>Lemon Jell-O cup, sugar-free</li></td>
		  <td>
		  <li>2-3 turkey and Swiss roll-ups</li>
		  <li>Baby carrots</li>
		  <li>Small plum</li></td>
		  <td>
		  <li>Turkey and Swiss sandwich</li>
		  <li>Pepper stips</li>
		  <li>Coleslaw or side salad</li>
		  <li>Raspberry Jell-O cup, sugar free</li></td>
		  <td>
		  <li>Tuna Salad</li>
		  <li>Sliced bell peppers</li>
		  <li>Orange Jell-O cup, sugar free</li></td>
		  <td>
		  <li>203 Muenster cheese and roast beef roll-ups</li>
		  <li>Italian coleslaw</li>
		  <li>Small peach</li></td>
		</tr>
		<tr>
		  <td><b>Midafternoon Snack</b></td>
		  <td><li>Pepper strips</li>
		  <li>2 ounces guacamole (1/4 cup)</li></td>
		  <td>
		  <li>1 stick light cheese</li>
		  <li>Baby carrots</li></td> 
		  <td>
		  <li>1-2 light cheese wedges</li>
		  <li>6 grape tomatoes</li></td>
		  <td>
		  <li>6 ounces blueberry light yogurt</li>
		  <li>10 cashews</li></td>
		  <td>
		  <li>1 clementine oragne</li>
		  <li>1-2 light cheese wedges</li></td>
		  <td>
		  <li>4-6 ounces strawberries</li>
		  <li>10 cashews</li></td>
		  <td>
		  <li>6 ounces strawberry light yogurt (nonfat)</li></td>
		</tr>
		<tr>
		  <td><b>Dinner</b></td>
		  <td><li>Grilled Chicken</li>
		  <li>1 cup mixed carrots, broccoli, and cauliflower blend</li>
		  <li>Salad: Romaine blend with Italian Dressing</li>
		  <li>Raspberry Jell-O cup, sugar free</li></td>
		  <td>
		  <li>Roasted sliced turkey</li>
		  <li>8 ounces sautéed carrots and 1 medium onion</li>
		  <li>Side salad topped with Italian Dressing</li>
		  <li>Lime Jell-O cup, sugar free</li></td> 
		  <td>
		  <li>1lb. extra-lean ground sirloin with seasoning</li>
		  <li>1 cup broccoli</li>
		  <li>Side salad with balsamic dressing</li>
		  <li>1-2 strawberry Jell-O cups, sugar-free</li></td>
		  <td>
		  <li>Pan-seared tilapia</li>
		  <li>mango-Melon Salsa</li>
		  <li>Fresh asparagus</li>
		  <li>Strawberry Jell-O cup, sugar free</li></td>
		  <td>
		  <li>Meat sauce spaghetti</li>
		  <li>Side salad (Italian, Oil and Vinegar, or Vinaigrette dressing)</li>
		  <li>Fudge bar</li></td>
		  <td>
		  <li>Chicken Piccata (No skin)</li>
		  <li>Green beans</li>
		  <li>Sliced tomatoes</li>
		  <li>Side salad with Italian Dressing</li>
		  <li>4-6 ounces raspberries on 1/2-1 cup of yogurt (nonfat)</li></td>
		  <td>
		  <li>Zucchini Lasagna</li>
		  <li>Salad: Lettuce, grape tomatoes, red cabbage and goat cheese with Oil and Vinegar dressing</li>
		  <li>Fudge bar or other low-fat ice cream bar</li></td>
		</tr>
	  </table>
	</div>
  </div>
</div>


<! http://chienna.wordpress.com/2012/03/01/extreme-korean-celebrity-diets/ >
<div class="tab-pane" id="weightLoss2">
  <div class="row">
	  <div class="panel panel-default">
	  <!-- Default panel contents -->
	  <div class="panel-heading"><font size="5"><b>Weight Loss Meal Plan 2</b></font></div>
	  <div class="panel-body">
	    <p><font size="3">This is more intense weight-loss meal plan, called 'Danish Diet'. It it known to be a diet plan that is used at the Royal Danish Hospital. This meal plan is very strictly high-protein and low-hydrocarbon diet for 14 days. The majority of the diet plan is composed of animal protein and black coffee. Repeat the meal schedule below twice over two weeks.</font></p>
	  </div>
	
	  <!-- Table -->
	  <table class="table">
	    <tr>
	      <td width="50">     </td>
		  <td width="100"><b>Day 1</b></td>
		  <td width="100"><b>Day 2</b></td> 
		  <td width="100"><b>Day 3</b></td>
		  <td width="100"><b>Day 4</b></td>
		  <td width="100"><b>Day 5</b></td>
		  <td width="100"><b>Day 6</b></td>
		  <td width="100"><b>Day 7</b></td>
		</tr>
		<tr>
		  <td><b>Breakfast</b></td>
		  <td>
		  <li>3 Hard-boiled Eggs</li>
		  <li>1 Grapefruit</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>1 Hard-boiled Egg</li>
		  <li>1 Grapefruit</li>
		  <li>Black Coffee</li></td> 
		  <td>
		  <li>1 Hard-boiled Egg</li>
		  <li>1 Grapefruit</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>1 Hard-boiled Egg</li>
		  <li>1 Grapefruit</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>1 Hard-boiled Egg</li>
		  <li>1 Grapefruit</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>2 Hard-boiled Eggs</li>
		  <li>1 Grapefruit</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>2 Hard-boiled Eggs</li>
		  <li>1 Grapefruit</li>
		  <li>Black Coffee</li></td>
		</tr>
		<tr>
		  <td><b>Lunch</b></td>
		  <td>
		  <li>2 Hard-boiled Eggs</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>2 Hard-boiled Eggs</li>
		  <li>1 Grapefruit</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>Vegetable Salad (Lettuce, Cucumber with Balsamic Vinegar of Lemon Dressing)</li>
		  <li>1 Grapefruit</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>Vegetable Salad (Lettuce, Cucumber with Balsamic Vinegar of Lemon Dressing)</li>
		  <li>1 Grapefruit</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>3 Hard-boiled Eggs</li>
		  <li>Boiled Spinach</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>Fruit Salad (Cucumber, Lettuce, Apple, Strawberry, Banana)</li></td>
		  <td>
		  <li>1 Chicken Breast</li>
		  <li>Celery</li>
		  <li>Tomato</li>
		  <li>1 Grapefruit</li>
		  <li>Black Coffee</li></td>
		</tr>
		<tr>
		  <td><b>Dinner</b></td>
		  <td>
		  <li>3 Hard-boiled Egg</li>
		  <li>Vegetable Salad (Lettuce, Cucumber with Balsamic Vinegar of Lemon Dressing)</li>
		  </td>
		  <td>
		  <li>Beef Steak 200g</li>
		  <li>Vegetable Salad (Lettuce, Cucumber with Balsamic Vinegar of Lemon Dressing)</li>
		  <li>Black Coffee</li>
		  </td>
		  <td>
		  <li>1 Hard-boiled Egg</li>
		  <li>1 Chicken Breast</li>
		  <li>Black Coffee</li>
		  </td>
		  <td>
		  <li>3 Hard-boiled Eggs</li>
		  <li>Boiled Spinach</li>
		  <li>Black Coffee</li>
		  </td>
		  <td>
		  <li>1 Piece of White Fish</li>
		  <li>Vegetable Salad (Lettuce, Cucumber with Balsamic Vinegar of Lemon Dressing)</li>
		  <li>1 Banana or 1 Toast</li>
		  <li>Black Coffee</li>
		  </td>
		  <td>
		  <li>Beef Steak 200g</li>
		  <li>Celery</li>
		  <li>Tomato</li>
		  <li>Black Coffee</li></td>
		  <td>
		  <li>1 Chicken Breast</li>
		  <li>Tomato</li>
		  <li>1 Grapefruit</li>
		  <li>Steamed Cabbage</li>
		  <li>Black Coffee</li>
		  </td>
		</tr>
	  </table>
	</div>
  </div>
</div>


</div>
        
      

@stop